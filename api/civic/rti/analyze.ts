import { analyzeRtiObjective } from '../../../server/services/rtiAgent';
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const stateOrUt = body.stateOrUt || '';
    const language = body.language || 'en';

    if (!userProblem || typeof userProblem !== 'string' || !userProblem.trim()) {
      return res.status(400).json({
        success: false,
        error: { code: 'VALIDATION_ERROR', message: 'userProblem is required for RTI analysis.' },
        requestId
      });
    }

    const result = await analyzeRtiObjective({
    ...
    console.error(`[Vercel RTI Analyze Error] [${requestId}]:`, err?.message || err);
