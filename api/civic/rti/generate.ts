import { generateRtiDraft } from '../../../server/services/rtiAgent';
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const result = await generateRtiDraft(body);
    return res.status(200).json({ ...result, requestId });
  } catch (err: any) {
    console.error(`[Vercel RTI Generate Error] [${requestId}]:`, err?.message || err);
